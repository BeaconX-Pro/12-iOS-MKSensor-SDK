...
@interface MKBXSSlotFrameTypePickView ()<UIPickerViewDataSource, UIPickerViewDelegate>
...
@property (nonatomic, assign)bxs_slotType frameType;

@end

@implementation MKBXSSlotFrameTypePickView
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component{
    ...
    if ([self.delegate respondsToSelector:@selector(bxs_slotFrameTypeChanged:)]) {
        [self.delegate bxs_slotFrameTypeChanged:row];
    }
}
...
- (UIImageView *)leftIcon{
    if (!_leftIcon) {
        ...
        _leftIcon.image = LOADICON(@"MKBXPSeriesSlathf", @"MKBXSSlotFrameTypePickView", @"bxs_slotFrameType.png");
    }
    return _leftIcon;
}
...
- (NSMutableArray *)dataList {
    if (!_dataList) {
        _dataList = [NSMutableArray arrayWithObjects:@"TLM",@"UID",@"URL",@"iBeacon",@"Sensor info",@"No Data", nil];
    }
    return _dataList;
}
