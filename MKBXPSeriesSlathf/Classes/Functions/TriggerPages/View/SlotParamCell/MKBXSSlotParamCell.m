...
@implementation MKBXSSlotParamCell

+ (MKBXSSlotParamCell *)initCellWithTableView:(UITableView *)tableView {
    MKBXSSlotParamCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKBXSSlotParamCellIdenty"];
    if (!cell) {
        cell = [[MKBXSSlotParamCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKBXSSlotParamCellIdenty"];
    }
    return cell;
}
...
- (void)rssiSliderValueChanged {
    ...
    if ([self.delegate respondsToSelector:@selector(bxs_slotParam_rssiChanged:)]) {
        [self.delegate bxs_slotParam_rssiChanged:[value integerValue]];
    }
}
...
- (void)txPowerSliderValueChanged {
    ...
    if ([self.delegate respondsToSelector:@selector(bxs_slotParam_txPowerChanged:)]) {
        [self.delegate bxs_slotParam_txPowerChanged:[value integerValue]];
    }
}

- (void)modeButtonPressed {
    if ([self.delegate respondsToSelector:@selector(bxs_slotParam_lowerPowerDetailPressed)]) {
        [self.delegate bxs_slotParam_lowerPowerDetailPressed];
    }
}
...
- (void)powerButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(bxs_slotParam_lowerPowerModeChanged:)]) {
        [self.delegate bxs_slotParam_lowerPowerModeChanged:self.powerButton.selected];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKBXSSlotParamCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKBXSSlotParamCellModel.class]) {
        return;
    }
    ...
    if (_dataModel.cellType == bxs_slotType_null) {
        //NO Data
        return;
    }
...
- (void)setupTxPowerParams {
    self.txPowerLabel.attributedText = [MKCustomUIAdopter attributedString:@[@"Tx power",@"   (-20,-16,-12,-8,-4,0,+3,+4,+6)"] fonts:@[MKFont(13.f),MKFont(12.f)] colors:@[DEFAULT_TEXT_COLOR,RGBCOLOR(223, 223, 223)]];
...
- (void)updateRssiMsg {
    if (self.dataModel.cellType == bxs_slotType_null || self.dataModel.cellType == bxs_slotType_tlm) {
        return;
    }
    if (self.dataModel.cellType == bxs_slotType_uid || self.dataModel.cellType == bxs_slotType_url) {
    ...
    if (self.dataModel.cellType == bxs_slotType_beacon) {
    ...
    if (self.dataModel.cellType == bxs_slotType_sensorInfo) {
...
- (void)addNormalSubViews {
    ...
    [self.backView addSubview:self.modeLabel];
    [self.backView addSubview:self.modeButton];
    [self.backView addSubview:self.powerButton];
    ...
    if (self.dataModel.powerModeIsOn) {
...
- (void)setupTopViews {
    ...
    [self.modeButton mas_remakeConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo(self.modeLabel.mas_right).mas_offset(5.f);
    ...
    [self.intervalField mas_remakeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.mas_equalTo(self.powerButton.mas_bottom).mas_offset(10.f);
        make.height.mas_equalTo(25.f);
    }];
...
- (UIImageView *)leftIcon{
    if (!_leftIcon) {
        ...
        _leftIcon.image = LOADICON(@"MKBXPSeriesSlathf", @"MKBXSSlotParamCell", @"bxs_slot_baseParams.png");
    }
    return _leftIcon;
}
...
- (UILabel *)modeLabel {
    if (!_modeLabel) {
        _modeLabel = [self loadLabelWithMsg:@"Low-power mode"];
    }
    return _modeLabel;
}
...
- (UIButton *)modeButton {
    if (!_modeButton) {
        ...
        [_modeButton setImage:LOADICON(@"MKBXPSeriesSlathf", @"MKBXSSlotParamCell", @"bxs_detailIcon.png") forState:UIControlStateNormal];
        [_modeButton addTarget:self
...
- (UIButton *)powerButton {
    if (!_powerButton) {
        ...
        [_powerButton setImage:LOADICON(@"MKBXPSeriesSlathf", @"MKBXSSlotParamCell", @"bxs_switchSelectedIcon.png") forState:UIControlStateNormal];
...
- (MKTextField *)intervalField {
    if (!_intervalField) {
        ...
        _intervalField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(bxs_slotParam_advIntervalChanged:)]) {
...
- (MKTextField *)advDurationField {
    if (!_advDurationField) {
        ...
        _advDurationField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(bxs_slotParam_advDurationChanged:)]) {
...
- (MKTextField *)standbyDurationField {
    if (!_standbyDurationField) {
        ...
        _standbyDurationField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(bxs_slotParam_standbyDurationChanged:)]) {
