...
@implementation MKBXSSlotUIDCellModel
@end

@interface MKBXSSlotUIDCell ()
...
@implementation MKBXSSlotUIDCell

+ (MKBXSSlotUIDCell *)initCellWithTableView:(UITableView *)tableView {
    MKBXSSlotUIDCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKBXSSlotUIDCellIdenty"];
    if (!cell) {
        cell = [[MKBXSSlotUIDCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKBXSSlotUIDCellIdenty"];
    }
    return cell;
}
...
#pragma mark - setter
- (void)setDataModel:(MKBXSSlotUIDCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKBXSSlotUIDCellModel.class]) {
        return;
    }
...
- (UIImageView *)leftIcon{
    if (!_leftIcon) {
        ...
        _leftIcon.image = LOADICON(@"MKBXPSeriesSlathf", @"MKBXSSlotUIDCell", @"bxs_slotAdvContent.png");
    }
    return _leftIcon;
}
...
- (MKTextField *)nameTextField {
    if (!_nameTextField) {
        ...
        _nameTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(bxs_advContent_namespaceIDChanged:)]) {
...
- (MKTextField *)instanceTextField {
    if (!_instanceTextField) {
        ...
        _instanceTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(bxs_advContent_instanceIDChanged:)]) {
