...
@implementation MKBXSSlotSensorInfoCellModel
@end

@interface MKBXSSlotSensorInfoCell ()
...
@implementation MKBXSSlotSensorInfoCell

+ (MKBXSSlotSensorInfoCell *)initCellWithTableView:(UITableView *)tableView {
    MKBXSSlotSensorInfoCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKBXSSlotSensorInfoCellIdenth"];
    if (!cell) {
        cell = [[MKBXSSlotSensorInfoCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKBXSSlotSensorInfoCellIdenth"];
    }
    return cell;
}
...
#pragma mark - setter
- (void)setDataModel:(MKBXSSlotSensorInfoCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKBXSSlotSensorInfoCellModel.class]) {
        return;
    }
...
- (UIImageView *)icon {
    if (!_icon) {
        ...
        _icon.image = LOADICON(@"MKBXPSeriesSlathf", @"MKBXSSlotSensorInfoCell", @"bxs_slotAdvContent.png");
    }
    return _icon;
}
...
- (MKTextField *)nameTextField {
    if (!_nameTextField) {
        ...
        _nameTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(bxs_advContent_tagInfo_deviceNameChanged:)]) {
...
- (MKTextField *)tagIDTextField {
    if (!_tagIDTextField) {
        ...
        _tagIDTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(bxs_advContent_tagInfo_tagIDChanged:)]) {
