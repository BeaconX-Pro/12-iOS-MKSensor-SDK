...
@implementation MKBXSSlotURLCellModel
@end

@interface MKBXSSlotURLCell ()
...
@implementation MKBXSSlotURLCell

+ (MKBXSSlotURLCell *)initCellWithTableView:(UITableView *)tableView {
    MKBXSSlotURLCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKBXSSlotURLCellIdenty"];
    if (!cell) {
        cell = [[MKBXSSlotURLCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKBXSSlotURLCellIdenty"];
    }
    return cell;
}
...
- (void)urlTypeLabelPressed {
    ...
    [pickView showPickViewWithDataList:self.urlTypeList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(bxs_advContent_urlTypeChanged:)]) {
            [self.delegate bxs_advContent_urlTypeChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKBXSSlotURLCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKBXSSlotURLCellModel.class]) {
        return;
    }
...
- (UIImageView *)leftIcon{
    if (!_leftIcon) {
        ...
        _leftIcon.image = LOADICON(@"MKBXPSeriesSlathf", @"MKBXSSlotURLCell", @"bxs_slotAdvContent.png");
    }
    return _leftIcon;
}
...
- (MKTextField *)textField {
    if (!_textField) {
        ...
        _textField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(bxs_advContent_urlContentChanged:)]) {
