...
@implementation MKBXSSlotBeaconCellModel
@end

@interface MKBXSSlotBeaconCell ()
...
@implementation MKBXSSlotBeaconCell

+ (MKBXSSlotBeaconCell *)initCellWithTableView:(UITableView *)tableView {
    MKBXSSlotBeaconCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKBXSSlotBeaconCellIdenty"];
    if (!cell) {
        cell = [[MKBXSSlotBeaconCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKBXSSlotBeaconCellIdenty"];
    }
    return cell;
}
...
#pragma mark - setter
- (void)setDataModel:(MKBXSSlotBeaconCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKBXSSlotBeaconCellModel.class]) {
        return;
    }
...
- (UIImageView *)leftIcon{
    if (!_leftIcon) {
        ...
        _leftIcon.image = LOADICON(@"MKBXPSeriesSlathf", @"MKBXSSlotBeaconCell", @"bxs_slotAdvContent.png");
    }
    return _leftIcon;
}
...
- (MKTextField *)majorTextField {
    if (!_majorTextField) {
        ...
        _majorTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(bxs_advContent_majorChanged:)]) {
...
- (MKTextField *)minorTextField {
    if (!_minorTextField) {
        ...
        _minorTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(bxs_advContent_minorChanged:)]) {
...
- (MKTextField *)uuidTextField {
    if (!_uuidTextField) {
        ...
        _uuidTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(bxs_advContent_uuidChanged:)]) {
