...
@implementation MKBXSTriggerSlotParamCellModel
@end

@interface MKBXSTriggerSlotParamCell ()
...
@implementation MKBXSTriggerSlotParamCell

+ (MKBXSTriggerSlotParamCell *)initCellWithTableView:(UITableView *)tableView {
    MKBXSTriggerSlotParamCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKBXSTriggerSlotParamCellIdenty"];
    if (!cell) {
        cell = [[MKBXSTriggerSlotParamCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKBXSTriggerSlotParamCellIdenty"];
    }
    return cell;
}
...
- (void)rssiSliderValueChanged {
    ...
    if ([self.delegate respondsToSelector:@selector(bxs_triggerSlotParam_rssiChanged:)]) {
        [self.delegate bxs_triggerSlotParam_rssiChanged:[value integerValue]];
    }
}
...
- (void)txPowerSliderValueChanged {
    ...
    if ([self.delegate respondsToSelector:@selector(bxs_triggerSlotParam_txPowerChanged:)]) {
        [self.delegate bxs_triggerSlotParam_txPowerChanged:[value integerValue]];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKBXSTriggerSlotParamCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKBXSTriggerSlotParamCellModel.class]) {
        return;
    }
...
- (void)setupTLMSliderUI {
    [self.txPowerLabel mas_remakeConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo(15.f);
        make.top.mas_equalTo(self.advDurationField.mas_bottom).mas_offset(15.f);
        make.right.mas_equalTo(-15.f);
        make.height.mas_equalTo(MKFont(15.f).lineHeight);
    }];
...
- (UIImageView *)leftIcon{
    if (!_leftIcon) {
        ...
        _leftIcon.image = LOADICON(@"MKBXPSeriesSlathf", @"MKBXSTriggerSlotParamCell", @"bxs_slot_baseParams.png");
    }
    return _leftIcon;
}
...
- (MKTextField *)intervalField {
    if (!_intervalField) {
        ...
        _intervalField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(bxs_triggerSlotParam_advIntervalChanged:)]) {
...
- (UILabel *)advDurationLabel {
    if (!_advDurationLabel) {
        _advDurationLabel = [self loadLabelWithMsg:@"Total adv duration"];
    }
...
- (MKTextField *)advDurationField {
    if (!_advDurationField) {
        ...
        _advDurationField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(bxs_triggerSlotParam_advDurationChanged:)]) {
