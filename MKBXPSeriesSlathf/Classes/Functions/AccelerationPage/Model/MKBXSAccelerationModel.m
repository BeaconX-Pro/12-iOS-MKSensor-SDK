...
@interface MKBXSAccelerationModel ()
...
@implementation MKBXSAccelerationModel
...
- (BOOL)readTriggerParams {
    ...
    [MKBXSInterface bxs_readThreeAxisDataParamsWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)readTriggerCount {
    __block BOOL success = NO;
    [MKBXSInterface bxs_readMotionTriggerCountWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configTriggerParams {
    __block BOOL success = NO;
    [MKBXSInterface bxs_configThreeAxisDataParams:self.samplingRate acceleration:self.scale motionThreshold:[self.threshold integerValue] sucBlock:^ {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
