...
@interface MKBXSAccelerationHeaderView ()
...
@implementation MKBXSAccelerationHeaderView
...
- (void)syncButtonPressed {
    ...
    [self.synIcon.layer removeAnimationForKey:@"bxs_synIconAnimationKey"];
    if ([self.delegate respondsToSelector:@selector(bxs_updateThreeAxisNotifyStatus:)]) {
        [self.delegate bxs_updateThreeAxisNotifyStatus:self.syncButton.selected];
    }
    if (self.syncButton.selected) {
        //开始旋转
        [self.synIcon.layer addAnimation:[MKCustomUIAdopter refreshAnimation:2.f] forKey:@"bxs_synIconAnimationKey"];
...
- (void)clearButtonPressed {
    if ([self.delegate respondsToSelector:@selector(bxs_clearMotionTriggerCountButtonPressed)]) {
        [self.delegate bxs_clearMotionTriggerCountButtonPressed];
    }
}
...
- (UIImageView *)synIcon {
    if (!_synIcon) {
        ...
        _synIcon.image = LOADICON(@"MKBXPSeriesSlathf", @"MKBXSAccelerationHeaderView", @"bxs_threeAxisAcceLoadingIcon.png");
    }
    return _synIcon;
}
