...
@implementation MKBXSAccelerationParamsCellModel
@end

@interface MKBXSAccelerationParamsCell ()
...
@implementation MKBXSAccelerationParamsCell

+ (MKBXSAccelerationParamsCell *)initCellWithTableView:(UITableView *)tableView {
    MKBXSAccelerationParamsCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKBXSAccelerationParamsCellIdenty"];
    if (!cell) {
        cell = [[MKBXSAccelerationParamsCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKBXSAccelerationParamsCellIdenty"];
    }
    return cell;
}
...
- (void)scaleButtonPressed {
    ...
    [pickView showPickViewWithDataList:self.scaleList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(bxs_accelerationParamsScaleChanged:)]) {
            [self.delegate bxs_accelerationParamsScaleChanged:currentRow];
        }
    }];
}
...
- (void)sampleRateButtonPressed {
    ...
    [pickView showPickViewWithDataList:self.sampleRateList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(bxs_accelerationParamsSamplingRateChanged:)]) {
            [self.delegate bxs_accelerationParamsSamplingRateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKBXSAccelerationParamsCellModel *)dataModel {
    _dataModel = nil;
...
- (void)updateThresholdUnit {
    if (self.scale == 0) {
        //2g
        self.thresholdUnitLabel.text = @"x16mg";
        return;
    }
    if (self.scale == 1) {
        //4g
        self.thresholdUnitLabel.text = @"x32mg";
        return;
    }
    if (self.scale == 2) {
        //8g
        self.thresholdUnitLabel.text = @"x62mg";
        return;
    }
    if (self.scale == 3) {
        //16g
        self.thresholdUnitLabel.text = @"x186mg";
        return;
    }
}
...
- (MKTextField *)textField {
    if (!_textField) {
        ...
        _textField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(bxs_accelerationMotionThresholdChanged:)]) {
