...
@interface MKBXSSettingController ()<UITableViewDelegate, 
...
@property (nonatomic, strong)MKBXSSettingModel *dataModel;

@end

@implementation MKBXSSettingController

- (void)dealloc {
    NSLog(@"MKBXSSettingController销毁");
}
...
- (void)mk_loraTextButtonCellSelected:(NSInteger)index
    ...
    if (index == 1) {
        //ADV Channel
        [self configAdvChannel:dataListIndex];
        return;
    }
}
...
- (void)loadSection0Datas {
    ...
    if (self.dataModel.supportThreeAcc || self.dataModel.supportTH || !self.dataModel.hallStatus) {
...
- (void)pushSensorConfigPage {
    MKBXSSensorConfigController *vc = [[MKBXSSensorConfigController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}

#pragma mark - 开关状态设置
- (void)pushQuickSwitchPage {
    MKBXSQuickSwitchController *vc = [[MKBXSQuickSwitchController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}
...
- (void)commandPowerOff{
    [[MKHudManager share] showHUDWithTitle:@"Setting..."
                              ...
                              isPenetration:NO];
    @weakify(self);
    [MKBXSInterface bxs_configPowerOffWithSucBlock:^ {
        [[MKHudManager share] hide];
...
- (void)loadSection1Datas {
    ...
    if ([MKBXSConnectManager shared].needPassword) {
    ...
    if (ValidStr([MKBXSConnectManager shared].password) && [MKBXSConnectManager shared].needPassword) {
...
- (void)setPasswordToDevice{
    ...
    [[MKHudManager share] showHUDWithTitle:@"Setting..."
                                     inView:self.view
                              isPenetration:NO];
    [MKBXSInterface bxs_configConnectPassword:password
                                     sucBlock:^{
        [[MKHudManager share] hide];
    }
                                  failedBlock:^(NSError * _Nonnull error) {
...
- (void)sendResetCommandToDevice{
    [[MKHudManager share] showHUDWithTitle:@"Setting..."
                                     inView:self.view
                              isPenetration:NO];
    [MKBXSInterface bxs_factoryResetWithSucBlock:^ {
        [[MKHudManager share] hide];
    } failedBlock:^(NSError *error) {
        [[MKHudManager share] hide];
        [self.view showCentralToast:error.userInfo[@"errorInfo"]];
...
- (void)loadSection3Datas {
    ...
    MKNormalTextCellModel *resetBatteryModel = [[MKNormalTextCellModel alloc] init];
    resetBatteryModel.leftMsg = @"Reset Battery";
    resetBatteryModel.showRightIcon = YES;
    resetBatteryModel.methodName = @"resetBattery";
    [self.section3List addObject:resetBatteryModel];
}

- (void)pushRemoteReminder {
    MKBXSRemoteReminderController *vc = [[MKBXSRemoteReminderController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}
...
- (void)sendResetBatteryCommandToDevice {
    [[MKHudManager share] showHUDWithTitle:@"Setting..."
                                     inView:self.view
                              isPenetration:NO];
    [MKBXSInterface bxs_batteryResetWithSucBlock:^{
        [[MKHudManager share] hide];
    } failedBlock:^(NSError * _Nonnull error) {
        [[MKHudManager share] hide];
        [self.view showCentralToast:error.userInfo[@"errorInfo"]];
...
- (void)configAdvChannel:(NSInteger)channel {
    [[MKHudManager share] showHUDWithTitle:@"Setting..."
                                     inView:self.view
                              isPenetration:NO];
    mk_bxs_advChannel channelValue = mk_bxs_advChannel_all;
    if (channel == 1) {
        channelValue = mk_bxs_advChannel_ch37;
    }else if (channel == 2) {
        channelValue = mk_bxs_advChannel_ch38;
    }else if (channel == 3) {
        channelValue = mk_bxs_advChannel_ch39;
    }
    
    [MKBXSInterface bxs_configADVChannel:channelValue sucBlock:^{
        [[MKHudManager share] hide];
        MKTextButtonCellModel *cellModel = self.section4List[1];
        cellModel.dataListIndex = channel;
        self.dataModel.advChannel = channel;
    } failedBlock:^(NSError * _Nonnull error) {
        [[MKHudManager share] hide];
        [self.view showCentralToast:error.userInfo[@"errorInfo"]];
        [self.tableView mk_reloadRow:1 inSection:4 withRowAnimation:UITableViewRowAnimationNone];
    }];
}
...
- (MKBXSSettingModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKBXSSettingModel alloc] init];
    }
    return _dataModel;
}
