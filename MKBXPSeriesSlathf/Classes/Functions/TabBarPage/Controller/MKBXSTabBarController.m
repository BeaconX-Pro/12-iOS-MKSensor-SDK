...
@interface MKBXSTabBarController ()
...
@implementation MKBXSTabBarController

- (void)dealloc {
    NSLog(@"MKBXSTabBarController销毁");
...
- (void)addNotifications {
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(gotoScanPage)
                                                 name:@"mk_bxs_popToRootViewControllerNotification"
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(centralManagerStateChanged)
                                                 name:mk_bxs_centralManagerStateChangedNotification
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(disconnectTypeNotification:)
                                                 name:mk_bxs_deviceDisconnectTypeNotification
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
...
- (void)gotoScanPage {
    ...
    [self dismissViewControllerAnimated:YES completion:^{
        @strongify(self);
        if ([self.delegate respondsToSelector:@selector(mk_bxs_needResetScanDelegate:)]) {
            [self.delegate mk_bxs_needResetScanDelegate:NO];
        }
    }];
}
...
- (void)dfuUpdateComplete {
    ...
    [self dismissViewControllerAnimated:YES completion:^{
        @strongify(self);
        if ([self.delegate respondsToSelector:@selector(mk_bxs_needResetScanDelegate:)]) {
            [self.delegate mk_bxs_needResetScanDelegate:YES];
        }
    }];
}
...
- (void)centralManagerStateChanged{
    ...
    if ([MKBXSCentralManager shared].centralStatus != mk_bxs_centralManagerStatusEnable) {
...
- (void)showAlertWithMsg:(NSString *)msg title:(NSString *)title{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"mk_bxs_needDismissAlert" object:nil];
    //让所有MKPickView消失
    ...
    [alertView showAlertWithTitle:title message:msg notificationName:@"mk_bxs_needDismissAlert"];
}

- (void)loadSubPages {
    MKBXSSlotController *slotPage = [[MKBXSSlotController alloc] init];
    slotPage.tabBarItem.title = @"SLOT";
    slotPage.tabBarItem.image = LOADICON(@"MKBXPSeriesSlathf", @"MKBXSTabBarController", @"bxs_slotTabBarItemUnselected.png");
    slotPage.tabBarItem.selectedImage = LOADICON(@"MKBXPSeriesSlathf", @"MKBXSTabBarController", @"bxs_slotTabBarItemSelected.png");
    MKBaseNavigationController *slotNav = [[MKBaseNavigationController alloc] initWithRootViewController:slotPage];

    MKBXSSettingController *settingPage = [[MKBXSSettingController alloc] init];
    settingPage.tabBarItem.title = @"SETTING";
    settingPage.tabBarItem.image = LOADICON(@"MKBXPSeriesSlathf", @"MKBXSTabBarController", @"bxs_settingTabBarItemUnselected.png");
    settingPage.tabBarItem.selectedImage = LOADICON(@"MKBXPSeriesSlathf", @"MKBXSTabBarController", @"bxs_settingTabBarItemSelected.png");
    MKBaseNavigationController *settingNav = [[MKBaseNavigationController alloc] initWithRootViewController:settingPage];

    MKBXSDeviceInfoController *devicePage = [[MKBXSDeviceInfoController alloc] init];
    devicePage.tabBarItem.title = @"DEVICE";
    devicePage.tabBarItem.image = LOADICON(@"MKBXPSeriesSlathf", @"MKBXSTabBarController", @"bxs_deviceTabBarItemUnselected.png");
    devicePage.tabBarItem.selectedImage = LOADICON(@"MKBXPSeriesSlathf", @"MKBXSTabBarController", @"bxs_deviceTabBarItemSelected.png");
