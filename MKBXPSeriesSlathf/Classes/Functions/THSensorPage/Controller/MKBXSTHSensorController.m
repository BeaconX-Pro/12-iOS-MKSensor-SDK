...
@interface MKBXSTHSensorController ()<UITableViewDelegate,
UITableViewDataSource,
mk_textSwitchCellDelegate,
MKTextFieldCellDelegate,
MKBXSTHSensorHeaderViewDelegate,
MKBXSSyncTimeCellDelegate>

@property (nonatomic, strong)MKBXSTHSensorHeaderViewModel *headerViewModel;

@property (nonatomic, strong)MKBXSTHSensorHeaderView *headerView;
...
@property (nonatomic, strong)MKBXSTHSensorModel *dataModel;

@end

@implementation MKBXSTHSensorController

- (void)dealloc {
    NSLog(@"MKBXSTHSensorController销毁");
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 3 && indexPath.row == 0) {
        //Export T&H data
        MKBXSExportHTDataController *vc = [[MKBXSExportHTDataController alloc] init];
...
#pragma mark - MKBXSTHSensorHeaderViewDelegate
...
- (void)receiveHTData:(NSNotification *)note {
    ...
    self.headerViewModel.humidity = dic[@"humidity"];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKBXPSeriesSlathf", @"MKBXSTHSensorController", @"bxs_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKBXSTHSensorHeaderViewModel *)headerViewModel {
    if (!_headerViewModel) {
        _headerViewModel = [[MKBXSTHSensorHeaderViewModel alloc] init];
    }
    return _headerViewModel;
}

- (MKBXSTHSensorHeaderView *)headerView {
    if (!_headerView) {
        _headerView = [[MKBXSTHSensorHeaderView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 200.f)];
...
- (MKBXSTHSensorModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKBXSTHSensorModel alloc] init];
    }
    return _dataModel;
}
