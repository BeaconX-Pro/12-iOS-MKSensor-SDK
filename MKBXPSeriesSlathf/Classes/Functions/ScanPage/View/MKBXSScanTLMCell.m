...
@implementation MKBXSScanTLMCellModel
@end

@interface MKBXSScanTLMCell ()
...
@implementation MKBXSScanTLMCell

+ (MKBXSScanTLMCell *)initCellWithTableView:(UITableView *)tableView{
    MKBXSScanTLMCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKBXSScanTLMCellIdenty"];
    if (!cell) {
        cell = [[MKBXSScanTLMCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKBXSScanTLMCellIdenty"];
    }
    return cell;
}
...
#pragma mark - setter
- (void)setDataModel:(MKBXSScanTLMCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKBXSScanTLMCellModel.class]) {
        return;
    }
    if (_dataModel.mvPerbit <= 100) {
        self.batteryLabel.text = [[NSString stringWithFormat:@"%ld",(long)_dataModel.mvPerbit] stringByAppendingString:@"%"];
    }else {
        self.batteryLabel.text = [[NSString stringWithFormat:@"%ld",(long)_dataModel.mvPerbit] stringByAppendingString:@"mV"];
    }
...
- (UIImageView *)leftIcon{
    if (!_leftIcon) {
        ...
        _leftIcon.image = LOADICON(@"MKBXPSeriesSlathf", @"MKBXSScanTLMCell", @"bxs_littleBluePoint.png");
    }
    return _leftIcon;
}
...
- (UILabel *)batteryMsgLabel{
    if (!_batteryMsgLabel) {
        ...
        _batteryMsgLabel.text = @"Battery level";
    }
