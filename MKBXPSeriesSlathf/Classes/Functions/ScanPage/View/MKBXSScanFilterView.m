...
static CGFloat const backViewHeight = 370.f;
...
@interface MKBXSScanFilterView ()<UIGestureRecognizerDelegate>
...
@property (nonatomic, strong)MKTextField *tagTextField;
...
@property (nonatomic, copy)void (^searchBlock)(NSString *searchName, NSString *searchTagID,NSInteger searchRssi);

@end

@implementation MKBXSScanFilterView

- (void)dealloc{
    NSLog(@"MKBXSScanFilterView销毁");
}
...
- (instancetype)init{
    if (self = [super init]) {
        ...
        [self.backView addSubview:self.tagTextField];
...
- (void)doneButtonPressed{
    ...
    [UIView animateWithDuration:0.25f animations:^{
        ...
        if (self.searchBlock) {
            self.searchBlock(SafeStr(self.nameTextField.text), SafeStr(self.tagTextField.text),(-100 - [value integerValue]));
        }
        if (self.superview) {
            [self removeFromSuperview];
        }
...
+ (void)showSearchName:(NSString *)name
    ...
    MKBXSScanFilterView *view = [[MKBXSScanFilterView alloc] init];
...
- (void)showSearchName:(NSString *)name
    ...
    self.tagTextField.text = SafeStr(tagID);
...
#define tagLabelPostion_Y (nameLabelPostion_Y + 30.f + 10.f)
#define textSpaceY (30.f + 10.f)
#define singalIconPostion_Y (tagLabelPostion_Y + textSpaceY + 25.f + 30.f)
...
- (UILabel *)tagLabel {
    if (!_tagLabel) {
        _tagLabel = [[UILabel alloc] initWithFrame:CGRectMake(offset_X,tagLabelPostion_Y , 100.f, 30.f)];
        _tagLabel.textColor = DEFAULT_TEXT_COLOR;
        _tagLabel.textAlignment = NSTextAlignmentLeft;
        _tagLabel.font = MKFont(14.f);
        _tagLabel.text = @"Tag ID";
    }
    return _tagLabel;
}

- (MKTextField *)tagTextField {
    if (!_tagTextField) {
        _tagTextField = [[MKTextField alloc] initWithTextFieldType:mk_normal];
        _tagTextField.frame  = CGRectMake(textFieldPostion_X,
                                          tagLabelPostion_Y,
                                           textFieldWidth,
                                           30.f);
        _tagTextField.maxLength = 12;
        _tagTextField.textColor = DEFAULT_TEXT_COLOR;
        _tagTextField.borderStyle = UITextBorderStyleNone;
        _tagTextField.font = MKFont(13.f);
        _tagTextField.textColor = DEFAULT_TEXT_COLOR;
        _tagTextField.placeholder = @"1-6 Bytes";
        _tagTextField.clearButtonMode = UITextFieldViewModeWhileEditing;
        _tagTextField.layer.masksToBounds = YES;
        _tagTextField.layer.borderColor = NAVBAR_COLOR_MACROS.CGColor;
        _tagTextField.layer.borderWidth = 0.5f;
        _tagTextField.layer.cornerRadius = 4.f;
    }
...
- (UILabel *)minRssiLabel {
    if (!_minRssiLabel) {
        _minRssiLabel = [[UILabel alloc] initWithFrame:CGRectMake(offset_X, tagLabelPostion_Y + textSpaceY, 100.f, 25.f)];
...
- (UIImageView *)signalIcon{
    if (!_signalIcon) {
        _signalIcon = [[UIImageView alloc] initWithFrame:CGRectMake(offset_X,
                                                                    ...
                                                                    signalIconWidth,
                                                                    signalIconHeight)];
        _signalIcon.image = LOADICON(@"MKBXPSeriesSlathf", @"MKBXSScanFilterView", @"bxs_wifiSignalIcon.png");
    }
    return _signalIcon;
}
...
- (UIImageView *)graySignalIcon {
    if (!_graySignalIcon) {
        _graySignalIcon = [[UIImageView alloc] initWithFrame:CGRectMake(backViewWidth - offset_X - signalIconWidth,
                                                                    ...
                                                                    signalIconWidth,
                                                                    signalIconHeight)];
        _graySignalIcon.image = LOADICON(@"MKBXPSeriesSlathf", @"MKBXSScanFilterView", @"bxs_wifiGraySignalIcon.png");
    }
