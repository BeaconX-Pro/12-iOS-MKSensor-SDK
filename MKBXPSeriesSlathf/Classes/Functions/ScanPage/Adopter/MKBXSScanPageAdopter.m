...
#pragma mark - *****************************MKBXSScanPageAdopter**********************


@implementation MKBXSScanPageAdopter

+ (NSObject *)parseBeaconDatas:(MKBXSBaseBeacon *)beacon {
    if ([beacon isKindOfClass:MKBXSiBeacon.class]) {
        //iBeacon
        MKBXSiBeacon *tempModel = (MKBXSiBeacon *)beacon;
    ...
    if ([beacon isKindOfClass:MKBXSUIDBeacon.class]) {
        //UID
        MKBXSUIDBeacon *tempModel = (MKBXSUIDBeacon *)beacon;
    ...
    if ([beacon isKindOfClass:MKBXSURLBeacon.class]) {
        //URL
        MKBXSURLBeacon *tempModel = (MKBXSURLBeacon *)beacon;
    ...
    if ([beacon isKindOfClass:MKBXSSensorInfoBeacon.class]) {
        //Sensor
        MKBXSSensorInfoBeacon *tempModel = (MKBXSSensorInfoBeacon *)beacon;
        MKBXSScanSensorInfoCellModel *cellModel = [[MKBXSScanSensorInfoCellModel alloc] init];
...
+ (MKBXSScanInfoCellModel *)parseBaseBeaconToInfoModel:(MKBXSBaseBeacon *)beacon {
    if (!beacon || ![beacon isKindOfClass:MKBXSBaseBeacon.class]) {
        return [[MKBXSScanInfoCellModel alloc] init];
    }
    MKBXSScanInfoCellModel *deviceModel = [[MKBXSScanInfoCellModel alloc] init];
    ...
    }else if (beacon.frameType == MKBXSProductionTestFrameType) {
        //如果是产测
        MKBXSProductionTestBeacon *tempModel = (MKBXSProductionTestBeacon *)beacon;
...
+ (void)updateInfoCellModel:(MKBXSScanInfoCellModel *)exsitModel beaconData:(MKBXSBaseBeacon *)beacon {
    ...
    if (beacon.frameType == MKBXSSensorInfoFrameType) {
        ...
        if (ValidStr(tempInfoModel.deviceName)) {
    ...
    NSObject *tempModel = [MKBXSScanPageAdopter parseBeaconDatas:beacon];
    if (!tempModel) {
        return;
    }
...
+ (UITableViewCell *)loadCellWithTableView:(UITableView *)tableView dataModel:(NSObject *)dataModel {
    ...
    if ([dataModel isKindOfClass:MKBXSScanSensorInfoCellModel.class]) {
        //Tag Info
        MKBXSScanSensorInfoCell *cell = [MKBXSScanSensorInfoCell initCellWithTableView:tableView];
        cell.dataModel = dataModel;
        return cell;
    }
    
    return [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKBXSScanPageAdopterIdenty"];
}
...
+ (CGFloat)loadCellHeightWithDataModel:(NSObject *)dataModel {
    ...
    if ([dataModel isKindOfClass:MKBXSScanSensorInfoCellModel.class]) {
        //Sensor Info
        MKBXSScanSensorInfoCellModel *tempModel = (MKBXSScanSensorInfoCellModel *)dataModel;
...
+ (NSInteger)fetchFrameIndex:(NSObject *)dataModel {
    ...
    if ([dataModel isKindOfClass:NSClassFromString(@"MKBXSScanSensorInfoCellModel")]) {
        //Sensor Info
        return 4;
    }
