...
#define textBackViewHeight (kViewHeight - kTopBarHeight - 170.f)

static CGFloat timeTextViewWidth = 130.f;
static CGFloat htTextViewWidth = 80.f;
static CGFloat htTextViewOffset_Y = 160.f;

#define textViewSpace (kViewWidth - 30.f - timeTextViewWidth - 2 * htTextViewWidth) / 4

@interface MKBXSExportHTDataController ()<MFMailComposeViewControllerDelegate,
...
- (void)dealloc {
    NSLog(@"MKBXSExportHTDataController销毁");
...
- (void)bxs_dateSelectedView_startPressed:(NSString *)startDate endDate:(NSString *)endDate {
    ...
    MKBXSFilterHTHistoryDataController *vc = [[MKBXSFilterHTHistoryDataController alloc] init];
...
- (NSString *)parseTemperatureHumidityData:(NSString *)content {
    ...
    for (NSInteger i = 0; i < total; i ++) {
        ...
        NSInteger tempHui = strtoul([[subContent substringWithRange:NSMakeRange(12, 4)] UTF8String],0,16);
